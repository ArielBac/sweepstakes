<div x-show="$store.menu.menuVisible" class="relative z-40 lg:hidden" role="dialog" aria-modal="true">
    ...
    <div x-show="$store.menu.menuVisible" x-transition:enter="transition-opacity ease-linear duration-300"
    ...
    <div class="fixed inset-0 z-40 flex">
        ...
        <div x-show="$store.menu.menuVisible" x-transition:enter="transition ease-in-out duration-300 transform"
            x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0"
            x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0"
            x-transition:leave-end="-translate-x-full"
            class="relative flex w-full max-w-xs flex-1 flex-col bg-white pt-5 pb-4">

            <div x-show="$store.menu.menuVisible" x-transition:enter="ease-in-out duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="ease-in-out duration-300" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="absolute top-0 right-0 -mr-12 pt-2">
                <button @click="$store.menu.menuVisible = false" type="button"
            ...
            <div class="flex flex-shrink-0 items-center px-4">
                <img class="h-12 w-auto" src="{{ asset('logo/logo.png') }}" alt="Sweepstake">
                <h2 class="text-lg font-bold leading-tight tracking-tight text-gray-800">
                    Sweepstake
                </h2>
            </div>
...
<div
    ...
    <div class="flex flex-shrink-0 items-center px-6">
        <img class="h-14 w-auto" src="{{ asset('logo/logo.png') }}" alt="Sweepstake">
        <h2 class="text-xl font-bold leading-tight tracking-tight text-gray-800">
            Sweepstake
        </h2>
    </div>
    <div class="mt-1 flex h-0 flex-1 flex-col overflow-y-auto">
        <nav class="mt-6 px-3">
            <div class="space-y-1">
                <a href="{{ route('sweepstakes.index') }}"
                    class="bg-gray-200 text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md"
